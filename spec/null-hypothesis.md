...
Wait for an answer. If the user cannot answer, guide them with the prompt in Step 3.

### Step 3 — Guided elicitation (if Step 2 produces no answer)

If the user cannot articulate the null hypothesis directly, use this structured elicitation:

**For trend stories** (the user wants to show that something has changed):
> "If the indicator showed no statistically meaningful change over the period,
> would the story still be publishable? What is the minimum change that would make it a story?"
...
> "If the correlation coefficient were near zero, or if the relationship reversed
> in a significant subset of the data, would the story still hold?
> What strength of association is required for the story to be credible?"
...
Once the null hypothesis is stated, derive from it the minimum analytical commitment.
...
The `methodological_floor` constrains the `data-design.md` phase:
the chosen method must be capable of testing it.
...
- **Story scoped down**: a subset of the data does show the effect. The report is scoped
...
- The null hypothesis must be specific enough to be testable. "The story is false if
  there is no effect" is not acceptable. State the minimum magnitude, direction or
  significance threshold.

- If the user refuses to state a null hypothesis ("I just want to see what the data shows"),
  record this in `notes.md` and flag it as an exploratory analysis.
  Exploratory analyses must carry an explicit disclaimer in the report's Scope Limit callout:
  "This analysis is exploratory. No hypothesis was pre-registered. Results should be interpreted
...
- [ ] `Methodological floor` specifies a concrete threshold (not "some effect" or "a significant result")
